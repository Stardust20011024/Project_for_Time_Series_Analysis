...
cor_data <- cor_data[complete.cases(cor_data), ]                                 ## 删除缺失行
...
ggsave("fig_acf.png", fig_acf, width = 8, height = 5, dpi = 300, bg = "white")
...
ggsave("fig_pacf.png", fig_pacf, width = 8, height = 5, dpi = 300, bg = "white")
...
fig_acf_diff <- ggplot(acf_diff_df, aes(x = lag, y = acf)) +
  ...
  theme_article +
  coord_cartesian(xlim = c(-1, max_lag + 1))
ggsave("fig_acf_diff.png", fig_acf_diff, width = 8, height = 5, dpi = 300, bg = "white")
...
fig_pacf_diff <- ggplot(pacf_diff_df, aes(x = lag, y = pacf)) +
  ...
  theme_article +
  coord_cartesian(xlim = c(0, max_lag + 1))
ggsave("fig_pacf_diff.png", fig_pacf_diff, width = 8, height = 5, dpi = 300, bg = "white")
...
cat("AIC du modèle:", AIC(auto_model), "\n")
cat("BIC du modèle:", BIC(auto_model), "\n")
print(auto_model)
